...
# PyInstaller spec — produce fleetwatcher-mcp-backend.exe for Tauri NSIS embedding.
# Fleet standard: strip=False, upx=False, noarchive=True (see tauri_nsis_building.md).
# Usage (from repo root):
#   uv run pyinstaller fleetwatcher-mcp-backend.spec --distpath dist --clean --noconfirm

block_cipher = None

a = Analysis(
    ["src/fleetwatcher_mcp/__main__.py"],
    pathex=["src"],
    binaries=[],
    datas=[("src/fleetwatcher_mcp", "fleetwatcher_mcp")],
    hiddenimports=[
        "uvicorn.logging",
        "uvicorn.loops",
